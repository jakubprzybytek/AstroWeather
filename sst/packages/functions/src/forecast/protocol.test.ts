...
describe("forecast protocol", () => {
  test("serializes six displays in the documented fixed order", () => {
    const body = serializeForecast("krakow", "2026-09-17T23:22:45.678", "2026-09-17T18:00:04", Array.from({ length: 6 }, (_, index) => display(index)));
    expect(body.split("\n").slice(0, 18)).toEqual([
      "protocol=1", "configurationId=krakow", "time=2026-09-17T23:22:45.678",
      "lastWeatherFetchTime=2026-09-17T18:00:04", "", "display=0", "board=num4x4_matrix5x21",
    ...
    expect(body).toContain("lastWeatherFetchTime=2026-09-17T18:00:04\n\ndisplay=0");
  ...
  test.each(["2026-09-17T18:00:04.000", "2026-09-17 18:00:04", ""])("rejects a malformed last weather fetch time %s", (value) => {
    expect(() => serializeForecast("krakow", "2026-09-17T23:22:45.678", value, Array.from({ length: 6 }, (_, index) => display(index))))
