...
| M-4 | Medium | Host Controller + all boards / PB8, J102/J104 pin 9, Q701 | **`LOW_POWER_ENABLE` is driven push-pull by every board.** The net is bussed to every board. The host's `main.c` configures PB8 as a push-pull output (driven low), and the DisplayController firmware used to share that configuration. As soon as the host drives it high, it shorts against five low outputs. Since 2026-09-24 the host drives it (switch 2 toggles low brightness); since 2026-09-25 the DisplayController is a separate CubeMX project whose `.ioc` leaves PB8 in analog mode. | Host `main.c` `MX_GPIO_Init`: `LOW_POWER_EN_Pin` is `GPIO_MODE_OUTPUT_PP`, reset level low. `DisplayController.ioc`: `LOW_POWER_ENABLE` is `GPIO_Analog`. | Next revision: add a series resistor (1k) at each board's PB8. | Confirmed; firmware fixed 2026-09-24, series resistor still open |
