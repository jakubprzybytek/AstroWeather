#pragma once

#include <Console/DisplayCommand.hpp>

namespace Device {
class Eeprom24AA04;
}

namespace Console {

// 'eeprom probe'              - acknowledge poll the chip
// 'eeprom dump'               - hex dump of the whole 512-byte array
// 'eeprom read <off> [len]'   - hex dump of a range (off/len are hex)
// 'eeprom write <off> <hex>'  - write a hex byte string, e.g. 'eeprom write 00 A55A01'
// 'eeprom erase'              - fill the array with 0xFF
// 'eeprom scan'               - ack-poll every address on the bus
CommandResult handleEepromCommand(const char* line, Device::Eeprom24AA04* eeprom);

} // namespace Console
